...
async function check() {
	const player = { minecraft_username: 'Merlinspace', minecraft_uuid: '00000000000000000000000000000001' }
	service.recordPresenceEvent({ type: 'join', ...player })
	assert.deepEqual((await service.listOnlinePlayers()).players.map(({ minecraftUsername }) => minecraftUsername), ['Merlinspace'])
