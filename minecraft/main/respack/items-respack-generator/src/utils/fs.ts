...
export async function writeJsonFile(destinationPath: string, value: unknown): Promise<void> {
	await ensureDirectory(path.dirname(destinationPath));
	await fs.writeFile(destinationPath, `${JSON.stringify(value, null, 2)}\n`, 'utf8');
}

export async function writeTextFile(destinationPath: string, value: string): Promise<void> {
	await ensureDirectory(path.dirname(destinationPath));
