...
export async function parseItemDefinition(
	...
	assertString(rawJson.modelType, `modelType in ${relativeDirectory}/item.json`);
	...
	switch (rawJson.modelType) {
		case 'basic': {
			assertExactKeys(rawJson, ['modelType', 'custom_model_data'], itemJsonPath);
		...
		case 'hat': {
			assertExactKeys(rawJson, ['modelType', 'custom_model_data', 'isTinted'], itemJsonPath);
			assertString(rawJson.custom_model_data, `${relativeDirectory} custom_model_data`);
		...
		case 'equippableCharm': {
			assertExactKeys(
				rawJson,
				['modelType', 'custom_model_data', 'isLeggings', 'equippable_asset_id'],
				itemJsonPath,
			);
			assertString(rawJson.custom_model_data, `${relativeDirectory} custom_model_data`);
		...
		default:
			throw new Error(
				`Unsupported modelType in ${itemJsonPath}: ${String(rawJson.modelType)}`,
			);
	}
}
