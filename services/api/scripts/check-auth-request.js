...
async function checkFlows() {
	...
	const grpc = {
		...
		upsertPendingJoin: async (request) => {
			pendingJoin = request;
			pendingJoinUpdates++;
		},
	};
	...
	const member = database.connection
		.insert(users)
		.values({
	...
	await assert.rejects(
		auth.createSignup('guest@example.com', '127.0.0.1'),
		/numeric @stu\.mmu\.ac\.uk address/,
	);
	await assert.rejects(
	...
	await assert.rejects(
		auth.addEmailToWhitelist({ id: member.id }, 'guest@example.com'),
		/responsible user/,
	);
	await auth.addEmailToWhitelist({ id: member.id }, 'Guest@Example.com', member.id);
	assert.deepEqual(
		auth.listEmailWhitelist().entries.map((entry) => entry.email),
		['guest@example.com'],
	);
	...
	assert.ok(
		(await auth.verifySignIn(signin.flowId, deliveredCodes.get('member@stu.mmu.ac.uk'))).token,
	);
	...
	const admin = database.connection
		...
		.returning({ id: users.id })
		.get();
	const managedPlayer = database.connection
		.insert(users)
		.values({
	...
	assert.equal(
		(
			await players.updateProfile(
				{ id: admin.id, isCommittee: true },
				String(managedPlayer.id),
				{
	...
	for (let attempt = 0; attempt < 5; attempt++) {
		await assert.rejects(
			auth.verifyMinecraftCode(signup.flowId, 'Not an item'),
			/Invalid Minecraft code/,
		);
	}
	await assert.rejects(
		auth.verifyMinecraftCode(signup.flowId, pendingJoin.code),
		/Minecraft code expired/,
	);
