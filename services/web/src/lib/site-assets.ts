...
export function getLandingVisuals() {
	const panoramas = JSON.parse(fs.readFileSync(path.join(publicRoot, 'assets', 'landing', 'panoramas', 'manifest.source.json'), 'utf8')) as {
...
export function getSiteVisuals() {
	...
	const backgrounds = fs.readdirSync(directory).filter((file) => /\.jpe?g$/i.test(file))
	return {
		background: `/assets/site-backgrounds/${encodeURIComponent(pick(backgrounds))}`,
		splash: getRandomSplash(),
	}
}

function getRandomSplash() {
	const splashes = fs.readFileSync(path.join(publicRoot, 'assets', 'landing', 'splashes.txt'), 'utf8')
		.split(/\r?\n/)
