...
When a committee member turns the server on for testing, the committee can move any online player between the two servers from Admin → Servers. During a live event, players can use the event page to move between main and the event server. Velocity moves all online players to the event server when the live route starts. The API checks the signed-in player's Minecraft UUID, ban state, event time, and server health before it accepts a player move. Velocity sends each move during its next three-second sync. A route change clears old manual moves.
...
On a mob kill, Killshift writes the killer UUID, mob ID, and event time to an outbox file in the persistent event world. It sends pending entries to `POST /api/internal/surprising-saturday/list-completion`. The API accepts only IDs in the event list. The outbox can replay a saved kill after an API outage or a server restart. On player join, Killshift reads `GET /api/internal/surprising-saturday/player-data/:uuid`. It refreshes the profile each minute. Chat shows the Minecraft username and role. The name tag shows the nickname and pronouns below the username. Committee members get moderator access after the API confirms their role. These calls use the internal bearer secret and do not use gRPC.
