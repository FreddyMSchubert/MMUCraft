...
import { DatabaseService, playerBans, playerProfiles, sessions, users } from '../database/database.service'
...
export class PlayerBansService {
	...
	list() {
		...
		return {
			bans: this.database.connection.select().from(playerBans).all()
				...
				.map((ban) => {
					...
					return {
						...
						color: effectivePlayerColor(player?.minecraft_uuid ?? null, profilesById.get(ban.user_id)?.color_hex),
	...
	remove(userId: number) {
		return this.database.connection.delete(playerBans)
			.where(eq(playerBans.user_id, userId)).run().changes === 1
	}

	resolve(userId: number, now = Date.now()): PlayerBanState {
		const ban = this.database.connection.select().from(playerBans)
			.where(eq(playerBans.user_id, userId)).get()
