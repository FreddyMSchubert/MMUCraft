...
All 3,085 explicit vanilla item references in the hopper catalogue were checked against the 26.3 client item assets. None were unknown. Data validation accepts 369 custom items and 280 filter groups.
...
28. **External resource packs do not yet advertise 26.3 support.** Kept the existing EvenBetterEnchants, Almost Vanilla Potions, and Fancy Beds pins. Inspected the merged files for removed fields and shader dependencies. All 1,130 JSON and metadata files parse. Client rendering remains a release check.
...
31. **Generated output needed to stay in sync.** Rebuilt the resource archive, its website copy, and generated server properties. The two ZIP files are identical and the generated SHA-1 agrees. ZIP timestamps can change the archive hash on a later rebuild, so compare the archive and generated settings from the same build.
...
41. **Current `main` added a Redstone Remote swing hook after the migration branch split.** The merge retained its old `ServerboundSwingPacket` target, which no longer exists in 26.3. Updated the mixin to inject into `handlePunch(ServerboundPunchPacket)`. The 26.3 client sends this packet for main-hand attacks, so the callback now passes `MAIN_HAND` explicitly. This preserves frequency changes for shift-left-click while avoiding right-click activation.
42. **Current `main` also added a map-resize check after the migration branch split.** Its bootstrap still called the removed `VanillaRegistries.createLookup()` method. Updated it to `createWorldLookup()`, matching the other 26.3 checks. The check still verifies every supported map size in both directions and the minimum and maximum limits.
43. **The command-line resource-pack merger initialized Swing on macOS.** The merge exposed this when the JVM aborted in AppKit before any files were merged. The utility only needs Swing in its optional GUI mode. The build script now starts its command-line invocation with `-Djava.awt.headless=true`. This fixes the root cause without changing the vendored merger or its GUI.
44. **The shaded merger build dirtied its submodule.** Maven writes `dependency-reduced-pom.xml` into the ResourcePackMerger checkout on every pack build. The build script now removes that generated file immediately after Maven finishes. This keeps normal pack builds from leaving unrelated untracked files.
45. **The Butterflies recipe could stop the server during registry loading.** The recipe used the misspelled namespace `mineraft:blue_dye`. Compilation and resource processing copy recipe JSON without resolving ingredient IDs, so those checks passed. The packaged-server run decoded the recipe registry, discarded the invalid `B` ingredient, and then rejected the shaped pattern because `B` was undefined. Corrected the ID to `minecraft:blue_dye` and added the recipe to the staging checklist.
...
| Data schema and staging | 369 items and 280 groups passed. Staged files match source data. |
| Hopper item IDs | All 3,085 explicit vanilla references exist in the target client assets. |
| Resource pack | Built; 1,130 JSON/metadata files parsed; metadata is 97.1; website copy and generated server hash match. |
...
- [ ] Hold a Redstone Remote and shift-left-click the air. Confirm its frequency decreases once per click. Hold the attack button against a block and confirm repeated punch packets do not produce unintended extra changes.
- [ ] Shift-left-click a block and an entity with the Redstone Remote. Confirm the frequency changes once, not twice. Right-click must still increase the frequency and must not also trigger the attack callback.
- [ ] Repeat the Redstone Remote checks in the off hand. The new punch packet is main-hand-only, so off-hand actions must not be mistaken for main-hand attacks.
...
- [ ] Craft Butterflies with blue, white, and pink dye, a dandelion, and a soul. Confirm the recipe loads, consumes the intended ingredients, and produces the correct decoration.
