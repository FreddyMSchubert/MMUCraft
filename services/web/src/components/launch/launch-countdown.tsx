...
export function LaunchCountdown({ compact = false, reloadAtZero = false }: { compact?: boolean; reloadAtZero?: boolean }) {
	...
	if (remainingSeconds !== null && remainingSeconds <= 0) return <span className={compact ? 'menuCountdown' : 'launchLive'}>Server is live!</span>

	const parts = splitDuration((remainingSeconds ?? 0) * 1000)
	if (compact) {
		return <span className="menuCountdown" aria-label={`Launch countdown: ${parts.days} days, ${parts.hours} hours, ${parts.minutes} minutes, ${parts.seconds} seconds`}>
			{parts.days}d {String(parts.hours).padStart(2, '0')}:{String(parts.minutes).padStart(2, '0')}:{String(parts.seconds).padStart(2, '0')}
		</span>
	}

	return <div className="launchCountdown" aria-label="Countdown to server launch">
		{Object.entries(parts).map(([label, value]) => <div className="launchCountdownSegment" key={label}>
			<strong>{label === 'days' ? String(value).padStart(3, '0') : String(value).padStart(2, '0')}</strong>
			<span>{label}</span>
		</div>)}
	</div>
}
