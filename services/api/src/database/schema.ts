...
export const velocitySettings = sqliteTable('velocity_settings', {
...
export const velocityServers = sqliteTable('velocity_servers', {
	id: integer('id').primaryKey({ autoIncrement: true }),
...
export const velocitySchedules = sqliteTable('velocity_schedules', {
	id: integer('id').primaryKey({ autoIncrement: true }),
	name: text('name').notNull(),
	server_id: integer('server_id').notNull().references(() => velocityServers.id),
	...
	check('velocity_schedules_window_check', sql`${table.ends_at_unix_ms} > ${table.starts_at_unix_ms}`),
])
