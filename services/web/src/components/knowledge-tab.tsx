...
const ADMONITION_ICONS: Record<AdmonitionType, string> = {
	...
	context: '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="m15.5 8.5-2 5-5 2 2-5 5-2Z"/></svg>',
}
...
const knowledgeMarkdown = new Marked({
	...
	extensions: [{
		...
		level: 'block',
		start(source) {
			return source.match(/^:::(?:info|warning|error|hint|tip|note|tldr|context)\b/m)?.index
		},
		tokenizer(source) {
			const match = /^:::(info|warning|error|hint|tip|note|tldr|context)(?:[ \t]+([^\r\n]+))?[ \t]*\r?\n([\s\S]*?)\r?\n:::[ \t]*(?:\r?\n|$)/.exec(source)
			if (!match) return
			...
			return {
				...
				raw: match[0],
				kind,
				title: match[2]?.trim() || kind.toUpperCase(),
...
export function KnowledgeTab({ pageId, onSelectPage }: {
	...
	const activePage = pages.find((page) => page.id === pageId) ?? flattenPages(visibleTree)[0] ?? null
	...
	const load = useCallback(async (options: { quiet?: boolean } = {}) => {
		...
		const selectedPage = allPages.find((page) => page.id === pageId) ?? flattenPages(filterUnlockedTree(knowledge.tree))[0]
		if (selectedPage) {
	...
	const renderedHtml = useMemo(() => {
		const html = stripDangerousHtml(knowledgeMarkdown.parse(pageMarkdown, { async: false }) as string)
		return html
	}, [pageMarkdown])
