...
Players can also switch during a live event from the Event tab. The API checks their website session and their online Minecraft UUID. Committee player moves use the same Velocity command queue. Velocity sends each command during its next three-second sync. A route change clears old manual moves.

## Event data

`surprising_saturday_events` stores the title, Markdown description, start and end times, scoring type, and scoring options as JSON. This draft accepts `list_completion`. The JSON value is an array of namespaced mob IDs, such as `minecraft:creeper`. The schema leaves room for other scoring types. Add validation and rendering for each new type before use.
...
Committee members create and edit future events in Admin → Surprising Saturday. They set the start and end times on the website. The API stores these times in SQLite. The public API hides the title and description before the start time. The Event tab shows a hidden upcoming card, a live page, and past events. The live page shows a server switch, a personal checklist, the top three players, and the full leaderboard. It renders Markdown and HTML in a sandboxed frame, so event HTML cannot run script in the website origin.

The Kill Shift template fills the title, rules, and 90 mob IDs from the Killshift coverage list. It does not set event times or create an event. The committee sets and edits the event times in the website form until the event starts.
