...
export function DynamicCountdowns() {
	...
	return <section className={`countdownGrid countdownGrid-${visible.length}`} aria-label="Upcoming events">
		{visible.map((countdown) => {
			...
			const style = {
				...
				'--countdown-background': hexWithAlpha(countdown.backgroundColor, countdown.backgroundAlpha),
				'--countdown-image': countdown.backgroundImageUrl ? `url(${JSON.stringify(countdown.backgroundImageUrl)})` : 'none',
			} as CSSProperties
			return <article className={`countdownCard${finished ? ' countdownCard-now' : ''}`} key={countdown.id} style={style}>
