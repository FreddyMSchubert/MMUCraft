...
export class MinecraftIdentityService {
	...
	resolveAndRefresh(uuidInput: string, usernameInput: string): UserRow | null {
		...
		if (uuid) {
			...
			try {
				...
				this.logger.warn(
					`Could not attach Minecraft UUID ${uuid} to legacy user ${legacy.id}: ${errorMessage(error)}`,
				);
				return null;
			}
		}
	...
	private refreshUsername(user: UserRow, username: string | null) {
		...
		if (conflict && conflict.id !== user.id) {
			this.logger.warn(
				`Refused Minecraft username refresh for UUID ${user.minecraft_uuid}: ${username} belongs to user ${conflict.id}`,
			);
			return;
		}
		...
		try {
			...
			this.logger.warn(
				`Could not refresh Minecraft username for user ${user.id}: ${errorMessage(error)}`,
			);
		}
	}
