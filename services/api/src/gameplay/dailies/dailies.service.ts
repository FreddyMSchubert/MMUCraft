...
export class DailiesService {
	...
	async getStatus(user: AuthenticatedUser) {
		...
		const advancementTask = await this.getOrPickAdvancementTarget(user, periodKey).catch(
			(error: unknown) => ({
				target: null,
				message:
		...
		const advancementCompleted =
			...
			Boolean(
				...
				(await this.isAdvancementCompleted(user, periodKey, advancementTask.target)),
			);
	...
	async claimLoginBonus(user: AuthenticatedUser) {
		...
		try {
			const result = await this.grantDailyLoginBonus(
	...
	async claimAdvancementBonus(user: AuthenticatedUser) {
		...
		try {
			const result = await this.claimDailyAdvancement(
				user.minecraftUsername,
				periodKey,
				now,
				picked.target,
	...
	async refreshForAdmin(userIdInput: string) {
		...
		const keptTasks = currentTasks.filter((task) =>
			this.hasClaimed(userId, task.task_id, periodKey),
		);
		...
		const advancementCompleted =
			this.hasClaimed(userId, ADVANCEMENT_BONUS_TASK_ID, periodKey) ||
			Boolean(
				currentAdvancement &&
		...
		const advancement = advancementCompleted
			? null
			: await this.pickDailyAdvancement(
					user.minecraftUsername,
					periodKey,
					now,
		...
		const generated =
			refreshedSlots.length === 0
				...
				: await this.generateMinecraftTasks(
						user,
						periodKey,
	...
	async claimDailyCompletion(user: AuthenticatedUser) {
		...
		if (
			this.completedDailyTaskCount(user.id, periodKey) !==
	...
	private async isAdvancementCompleted(
		...
		const recorded = this.database.connection
		...
		return this.claimDailyAdvancement(
			user.minecraftUsername,
			periodKey,
			Date.now(),
			target,
	...
	private async grantDailyLoginBonus(
	...
	private async generateMinecraftTasks(
		user: Pick<AuthenticatedUser, 'id' | 'minecraftUsername'>,
		periodKey: string,
	...
	private async pickDailyAdvancement(
		minecraftUsername: string,
		periodKey: string,
		unixMs: number,
		...
		return await callUnary<PickDailyAdvancementResponse>(
			this.getGameplayControlClient(),
			'PickDailyAdvancement',
			{
				minecraft_username: minecraftUsername,
				period_key: periodKey,
				unix_ms: unixMs,
...
function calculateLoginStreak(
	...
	const missedDaysBeforeToday = Math.max(
		0,
		daysBetweenPeriodKeys(previousKey, currentPeriodKey) - 1,
	);
...
function rewardForStreak(streak: number) {
	...
	return LOGIN_BONUS_REWARDS[rewardIndex] ?? LOGIN_BONUS_REWARDS[0];
}
