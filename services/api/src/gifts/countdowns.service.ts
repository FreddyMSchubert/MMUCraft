...
export class CountdownsService {
	...
	create(input: CountdownInput) {
		...
		this.database.connection.transaction((tx) => {
			...
			if (existing.length >= 4)
				throw new BadRequestException('You can have at most 4 countdowns');
			createdId = Number(
				tx
					.insert(countdowns)
					.values({
		...
		const created = this.database.connection
			.select()
			.from(countdowns)
			.where(eq(countdowns.id, createdId))
			.get();
		if (!created) throw new NotFoundException('Countdown not found');
	...
	update(idInput: string, input: CountdownInput) {
		...
		const updated = this.database.connection
			...
			.where(eq(countdowns.id, id))
			.get();
		if (!updated) throw new NotFoundException('Countdown not found');
		...
		this.database.connection.transaction((tx) => {
			...
			tx.select()
				...
				.forEach((row, position) =>
					tx.update(countdowns).set({ position }).where(eq(countdowns.id, row.id)).run(),
				);
		});
		return { ok: true };
	}
	...
	move(idInput: string, directionInput: 'up' | 'down' | undefined) {
		...
		this.database.connection.transaction((tx) => {
			...
			if (!current) throw new NotFoundException('Countdown not found');
			const swap = rows[swapIndex];
			if (!swap) return;
			tx.update(countdowns)
				.set({ position: swap.position })
				.where(eq(countdowns.id, id))
				.run();
			tx.update(countdowns)
				.set({ position: current.position })
				.where(eq(countdowns.id, swap.id))
				.run();
		});
		return this.list();
	}
...
function nextLondonMidnight(timestamp: number) {
	...
	return parseLondonDateTime(
		`${nextDay.getUTCFullYear()}-${two(nextDay.getUTCMonth() + 1)}-${two(nextDay.getUTCDate())}T00:00`,
	);
}
