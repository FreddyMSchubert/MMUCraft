...
export const users = sqliteTable(
	'users',
	{
		...
		responsible_user_id: integer('responsible_user_id').references(
...
export const emailSendEvents = sqliteTable(
	'email_send_events',
	{
		id: text('id').primaryKey(),
...
export const playerProfiles = sqliteTable('player_profiles', {
	...
	emoji_override_json: text('emoji_override_json'),
...
export const claims = sqliteTable(
	...
	(table) => [
		uniqueIndex('claims_dimension_chunk_unique').on(
			table.dimension,
			table.chunk_x,
			table.chunk_z,
		),
...
export const claimMembers = sqliteTable(
	'claim_members',
	{
...
export const playerMoneyEvents = sqliteTable(
	'player_money_events',
	{
		id: text('id').primaryKey(),
		user_id: integer('user_id')
...
export const knowledgeUnlocks = sqliteTable(
	'knowledge_unlocks',
	{
		user_id: integer('user_id')
			.notNull()
...
export const knowledgeReads = sqliteTable(
	'knowledge_reads',
	{
		user_id: integer('user_id')
			.notNull()
...
export const shopUnlocks = sqliteTable(
	'shop_unlocks',
	{
		user_id: integer('user_id')
			.notNull()
...
export const dailyAdvancementTargets = sqliteTable(
	'daily_advancement_targets',
	{
		user_id: integer('user_id')
			.notNull()
			.references(() => users.id, { onDelete: 'cascade' }),
...
export const playerBans = sqliteTable(
	...
	(table) => [
		check(
			'player_bans_expiry_check',
			sql`${table.expires_at_unix_ms} is null or ${table.expires_at_unix_ms} > ${table.created_at_unix_ms}`,
		),
	],
);
...
export const dailyTasks = sqliteTable(
	...
	(table) => [
		...
		uniqueIndex('daily_tasks_user_period_task_unique').on(
			table.user_id,
			table.period_key,
			table.task_id,
		),
...
export const giftCodes = sqliteTable(
	'gift_codes',
	{
		code: text('code').primaryKey(),
		amount_dabloons: integer('amount_dabloons').notNull(),
		redemption_mode: text('redemption_mode', { enum: ['single', 'per_user'] })
			.notNull()
			.default('single'),
	...
	(table) => [
		...
		check(
			'gift_codes_redemption_mode_check',
			sql`${table.redemption_mode} in ('single', 'per_user')`,
		),
		check('gift_codes_members_only_check', sql`${table.members_only} in (0, 1)`),
		check(
			'gift_codes_redeemed_pair_check',
			sql`(${table.redeemed_by_user_id} is null and ${table.redeemed_at_unix_ms} is null) or (${table.redeemed_by_user_id} is not null and ${table.redeemed_at_unix_ms} is not null)`,
		),
	],
);

export const giftCodeRedemptions = sqliteTable(
...
export const countdowns = sqliteTable(
	...
	(table) => [
		...
		check(
			'countdowns_background_alpha_check',
			sql`${table.background_alpha} between 0 and 100`,
		),
	],
);
