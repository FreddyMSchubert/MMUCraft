...
export function effectivePlayerColor(
...
function setBrightness(rgb: number, brightness: number) {
	...
	if (saturation !== 0) {
		...
		hue =
			(red === max
				? blueDistance - greenDistance
				: green === max
					? 2 + redDistance - blueDistance
	...
	return hex(
		...(channels.map((channel) => Math.round(channel * 255)) as [number, number, number]),
	);
}
