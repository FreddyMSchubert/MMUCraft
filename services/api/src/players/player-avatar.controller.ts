import { BadGatewayException, BadRequestException, Controller, Get, Header, NotFoundException, Param, StreamableFile } from '@nestjs/common'
...
type CachedAvatar = { expiresAt: number; png: Promise<Buffer> }
...
export class PlayerAvatarController {
	...
	async discordAvatar(@Param('uuid') uuidInput: string) {
		...
		const png = await this.getCached(this.discordAvatars, `${uuid}:${color}`, async () => {
			return this.addDiscordRing(await this.getHead(uuid, user?.minecraft_username ?? ''), color)
		})
		return new StreamableFile(png)
	}

	private findUser(uuid: string) {
		return this.database.connection.select().from(users).where(eq(users.minecraft_uuid, uuid)).get()
	}
	...
	private async getCached(cache: Map<string, CachedAvatar>, key: string, generate: () => Promise<Buffer>) {
		const cached = cache.get(key)
		if (cached && cached.expiresAt > Date.now()) return cached.png
		if (cached) cache.delete(key)
		if (cache.size >= AVATAR_CACHE_SIZE) cache.delete(cache.keys().next().value!)

		const png = generate().catch((error) => {
			cache.delete(key)
	...
	private async generateHead(uuid: string, fallbackName: string) {
		...
		const hat = await skin.clone().extract({ left: 40, top: 8, width: 8, height: 8 })
			.png().toBuffer()
		return sharp(face).composite([{ input: hat }]).resize(128, 128, { kernel: 'nearest' }).png().toBuffer()
	}
	...
	private async addDiscordRing(head: Buffer, color: string) {
		...
		const playerHead = await sharp(head).resize(108, 108, { kernel: 'nearest' }).png().toBuffer()
		return sharp({
