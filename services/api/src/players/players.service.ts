...
export class PlayersService {
	...
	async updateProfile(
		...
		const now = Date.now();
		const profile = this.normalizeProfileInput(
			input,
			this.getProfile(target.id).showDeathCounter,
		);
	...
	private normalizeProfileInput(
		...
		return {
			preferredName: sanitizeProfileText(
				input.preferredName,
				PROFILE_TEXT_LIMITS.preferredName,
				'Nickname',
			),
			pronouns: sanitizeProfileText(input.pronouns, PROFILE_TEXT_LIMITS.pronouns, 'Pronouns'),
			courseYear: sanitizeProfileText(
				input.courseYear,
				PROFILE_TEXT_LIMITS.courseYear,
				'Course / Year',
			),
			discordUsername: sanitizeProfileText(
	...
	private async applyPlayerColor(minecraftUuid: string, color: string) {
		const response = await callUnary<{ applied: boolean }>(
			this.getGameplayControlClient(),
			'ApplyPlayerColor',
			{ minecraft_uuid: minecraftUuid, color_hex: color },
		);
	...
	private normalizeMinecraftStatInput(
...
export async function fetchMojangProfileByUuid(
	...
	const texturesProperty = profileBody?.properties?.find(
