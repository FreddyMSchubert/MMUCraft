...
export function adoptLegacyDatabase(databasePath: string, migrationsFolder: string) {
	if (!existsSync(databasePath) || statSync(databasePath).size === 0 || isDrizzleDatabase(databasePath)) {
		return
	}
...
function isDrizzleDatabase(databasePath: string) {
	...
	try {
		...
		try {
			const appliedMigration = database.select({ id: drizzleMigrations.id })
...
function copyLegacyRows(
	...
	try {
		...
		const userRows = readWithFallback(
			...
			}).from(users).all().map((row) => ({ ...row, is_member: 0, is_committee: 0 })),
		)
		...
		const redemptionRows = readOptional(() => legacy.select().from(giftCodeRedemptions).all())
			.map((row) => ({ ...row, code: row.code.toLowerCase() }))
		...
		target.transaction((tx) => {
			...
			insertRows(tx, playerMoneyEvents, readOptional(() => legacy.select().from(playerMoneyEvents).all()))
			insertRows(tx, knowledgeUnlocks, readOptional(() => legacy.select().from(knowledgeUnlocks).all()))
			...
			for (const gift of giftRows) {
				if (gift.redeemed_by_user_id !== null && gift.redeemed_at_unix_ms !== null) {
					tx.insert(giftCodeRedemptions).values({
						code: gift.code,
...
function insertRows(
	database: Parameters<Parameters<ReturnType<typeof drizzle<typeof schema>>['transaction']>[0]>[0],
