...
export class ShopService implements OnModuleInit {
	...
	getShopForUser(user: AuthenticatedUser) {
		...
		return {
			...
			items: visibleItems.map((item) => {
				const dailyDiscountPercent = dailyDealIds.has(item.id)
					? this.getDailyDiscountPercent(item.id)
					: 0;
				...
				return {
					...
					discountedPriceDabloons: this.discountedPrice(
						item.priceDabloons,
						discountPercent,
					),
					...
					available: this.isAvailableForPurchase(
						user.id,
						item,
						availability,
						unlockedIds,
	...
	async purchaseItem(user: AuthenticatedUser, itemIdInput: unknown) {
		...
		if (!purchase.purchased) {
			throw new BadRequestException(
				purchase.message || this.purchaseFailureMessage(purchase),
			);
		}
	...
	getUnlockAvailabilityForMinecraftUsername(
		minecraftUuidInput: string,
		minecraftUsernameInput: string,
	) {
		const minecraftUsername = minecraftUsernameInput.trim();
	...
	private getUnlockedItemIds(userId: number, type?: ShopItemType): Set<string> {
		const rows = this.database.connection
			.select({ item_id: shopUnlocks.item_id })
			.from(shopUnlocks)
			.where(
				type
	...
	private async purchaseFromMod(
		minecraftUsername: string,
		item: CatalogItem,
		priceDabloons: number,
	): Promise<PurchaseShopItemResponse> {
		return await callUnary<PurchaseShopItemResponse>(
			this.getGameplayControlClient(),
			'PurchaseShopItem',
			{
	...
	private async getCharmInventoryFromMod(
		minecraftUsername: string,
	): Promise<GetCharmInventoryResponse> {
	...
	private async upgradeCharmFromMod(
		...
		return await callUnary<UpgradeCharmResponse>(
			this.getGameplayControlClient(),
			'UpgradeCharm',
			{
				minecraft_username: minecraftUsername,
		...
		for (const filePath of this.findItemJsonFiles(root)) {
			...
			if (item) {
				...
				const textureFilePath =
					this.findModelTextureFilePath(directory) ??
					this.findFlatTextureFilePath(parsed.id, directory, root);
				assets.set(parsed.id, {
					animation: textureFilePath
						? this.readAnimationDefinition(textureFilePath)
						: null,
					modelFilePath,
	...
	private parseCatalogItem(
		json: FakeItemDefinition,
		directory: string,
		root: string,
	): CatalogItem | null {
	...
	private getDailyDealIds(items: CatalogItem[]): Set<string> {
		...
		return new Set(
			[...items]
				.sort(
					(left, right) =>
						this.seededRank(`${date}:${left.id}`) -
						this.seededRank(`${date}:${right.id}`),
				)
	...
	private getDailyDiscountPercent(itemId: string): number {
		const roll = this.seededRank(`${this.getDailyDealDate()}:discount:${itemId}`);
	...
	private getDailyDealMessage(item: CatalogItem): string {
		const index =
			this.seededRank(`${this.getDailyDealDate()}:message:${item.id}`) %
			DAILY_DEAL_MESSAGES.length;
		return (DAILY_DEAL_MESSAGES[index] ?? DAILY_DEAL_MESSAGES[0]).replaceAll(
			'{item}',
			item.title,
		);
	}
	...
	private findFlatTextureFilePath(
		itemId: string,
				...
				? parsed.animation.frames
						.map((frame) =>
							typeof frame === 'number'
								...
								: frame &&
									  typeof frame === 'object' &&
									  typeof (frame as { index?: unknown }).index === 'number'
									? (frame as { index: number }).index
									: null,
						)
						.filter(
							(frame): frame is number =>
								frame !== null && Number.isInteger(frame) && frame >= 0,
						)
				: null;
