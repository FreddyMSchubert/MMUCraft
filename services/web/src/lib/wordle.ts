...
export interface WordleGuess { word: string; result: TileResult[] }
...
export function getDailyAnswer(words: readonly string[], dateKey: string) {
	...
	const dayNumber = Math.floor(Date.UTC(year, month - 1, day) / MS_PER_DAY) + WORD_CYCLE_DAY_OFFSET
...
function zonedTimeToUtc(parts: { year: number; month: number; day: number; hour: number; minute: number; second?: number }) {
	const base = Date.UTC(parts.year, parts.month - 1, parts.day, parts.hour, parts.minute, parts.second ?? 0)
	let utc = base
